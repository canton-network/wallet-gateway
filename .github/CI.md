...
| `test-migrations`      | SQL migration tests                 | Conditional                                             | `needs: build`                                                     | `yarn nx affected -t test:migrations --base=origin/${{ github.base_ref }} --head=HEAD --parallel=1`; only runs in projects that define a `test:migrations` script (currently `@canton-network/core-wallet-store-sql` and `@canton-network/core-signing-store-sql`) |
| `check-migration-lock` | Test SQL migration immutability     | Conditional                                             | `needs: build`                                                     | `yarn nx affected -t test:migrations --base=origin/${{ github.base_ref }} --head=HEAD --parallel=1`; only runs in projects that define a `test:migrations` script (currently `@canton-network/core-wallet-store-sql` and `@canton-network/core-signing-store-sql`) |
